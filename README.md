...
**Example — quick ablation (no augmentation):**
```bash
...
Prints a full console report and generates all plots and JSON report in `results/`.
...
| `--share` | Generate a temporary public Gradio URL (72h) |

The app features:
- Drag-and-drop or click-to-upload image input
- Top-5 confidence bar chart rendered in real time
- Live prediction history sidebar
...
```
Raw CIFAR-10 (50,000 train)
        │
        ├──► 45,000 Training samples  ──► Augmentation ──► Model
        │
...
|-----------|---------------|-----------|
| Optimiser | SGD + Nesterov(0.9) | Consistently outperforms Adam on CIFAR-10 |
| Initial LR | 0.1 | Aggressive start; cosine decay handles the rest |
| LR Schedule | CosineAnnealing(T=100, η_min=1e-5) | Smooth decay; no manual milestones |
| Weight Decay | 5e-4 | Standard L2 for CIFAR-scale models |
| Label Smoothing | ε = 0.1 | Improves calibration; ~0.5% accuracy gain |
| Gradient Clip | max_norm = 5.0 | Prevents occasional spike instability |
| Mixed Precision | AMP fp16/fp32 | 2–3× throughput on RTX GPUs with no accuracy loss |
...
The model converged smoothly over 100 epochs. Best validation accuracy of **95.06%** was achieved at **epoch 95**, after which the cosine schedule had reduced the learning rate to near-zero. Train/validation accuracy curves track closely throughout training — the regularisation strategy (dropout + weight decay + label smoothing + augmentation) effectively prevents overfitting despite the 11M parameter count.

### Calibration

An **ECE of 0.0697** indicates reasonable calibration. The reliability diagram shows mild overconfidence in the 0.5–0.8 confidence range — a common behaviour in cross-entropy-trained classifiers. Label smoothing (ε=0.1) partially mitigates this. Mean confidence on correct predictions (89.26%) is substantially higher than on incorrect ones (67.19%), confirming the confidence score is a reliable uncertainty signal.

### Error Analysis

The dominant failure mode is **cat/dog confusion**:
- 70 cats (7.0%) misclassified as dogs
- 50 dogs (5.0%) misclassified as cats

Inspection of the top-20 most confident mistakes reveals these are genuinely hard examples — ambiguous poses, unusual angles, or heavy occlusion — where even human annotators might hesitate. Vehicle classes (automobile 98.2%, truck 97.1%, ship 96.6%) achieve the highest accuracy, benefiting from their rigid, distinctive shapes.
...
All hyperparameters used during training are embedded inside every checkpoint file and logged to `results/training_history.json`.
